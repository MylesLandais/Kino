...
| Agent process layer | `Adaptation.AgentProcess` under `Kino.Agent.DynamicSupervisor` |
...
3. **Supervision** — `Kino.Agent.Supervisor` (child of `Kino.Supervisor`) owns `Kino.Agent.Registry` and `Kino.Agent.DynamicSupervisor`. Agents are `restart: :transient`. Training uses Oban queue `adaptation` (concurrency 1), not the agent DynamicSupervisor.
